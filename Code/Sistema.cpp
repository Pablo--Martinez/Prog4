...
void iniciarSesion(){
	...
	cout << "Ingrese la contraseÃ±a: ";cin >> pass;

	TSesion ts;
	try{
		ts = cu->iniciarSesion(ci);

		if((ts == APD)||(ts == CM)){ //ComÃºn o administrativo

			  escorrecta = cu->ingresarContrasenia(pass);
			  if (escorrecta){
					  cu->asignarSesion();
			  }else{
				  throw std::invalid_argument("La contraseÃ±a no es correcta.");
			  }
...
void altaReactivacionUsuario(){
	...
	if (cu->ingresoCI(ci_usuario)){ //Si existe
		...
		cout << "Â¿Desea ingresar otra categoria al Usuario?(S/N) "; cin >> otro_rol;
		if(otro_rol == "S"){
			string rol_usuario2;
		...
		string Confirmar;
		cout << "Â¿Confirmar alta de Usuario? (S/N): ";
		cin >> Confirmar;
		if (Confirmar == "S"){
